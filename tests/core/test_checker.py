...
def test_that_a_password_with_a_length_of_7_or_more_is_strong() -> None:
    password_strength = check_strength(valid_user)
    assert password_strength[0] == PasswordStrength.STRONG


def test_that_a_password_with_a_length_of_6_or_less_is_weak() -> None:
...
def test_that_a_password_with_1_or_more_characters_is_strong() -> None:
    password_strength = check_strength(valid_user)
    assert password_strength[0] == PasswordStrength.STRONG


def test_that_a_password_with_no_characters_is_weak() -> None:
    password_strength = check_strength(all_numbers)
    assert password_strength[0] == PasswordStrength.WEAK
    assert NO_LETTERS in password_strength[1]


def test_that_a_password_with_1_or_more_number_is_strong() -> None:
    password_strength = check_strength(valid_user)
    assert password_strength[0] == PasswordStrength.STRONG


def test_that_a_password_with_no_numbers_is_weak() -> None:
    password_strength = check_strength(all_characters)
    assert password_strength[0] == PasswordStrength.WEAK
    assert NO_NUMBERS in password_strength[1]


def test_that_admin_password_with_a_length_of_10_or_more_is_strong() -> None:
    password_strength = check_strength(valid_admin, UserType.ADMIN)
    assert password_strength[0] == PasswordStrength.STRONG


def test_that_admin_password_with_a_length_of_9_or_less_is_weak() -> None:
    password_strength = check_strength(short_length, UserType.ADMIN)
    assert password_strength[0] == PasswordStrength.WEAK
    assert LESS_THEN_10 in password_strength[1]


def test_that_admin_password_with_a_special_character_is_strong() -> None:
    password_strength = check_strength(valid_admin, UserType.ADMIN)
    assert password_strength[0] == PasswordStrength.STRONG


def test_that_admin_password_with_no_special_character_is_weak() -> None:
